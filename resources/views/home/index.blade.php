...
@section('content')
    <?php
        $reasons = [
            'সরকার নিবন্ধিত প্রতিষ্ঠান',
            'সবচেয়ে কম সময়ে সর্বাধিক চুক্তিবদ্ধ প্রতিষ্ঠান',
            'প্রতিমাসে দুইবার প্রতিষ্ঠানে প্রতিনিধির সার্ভিস',
            'প্রতিটি অঞ্চলে আঞ্চলিক সার্ভিস প্রতিনিধি',
            'সাশ্রয়ী মূল্যে সম্পূর্ণ ডিজিটাল স্কুল',
            'কোন প্রভাব নয়, গ্রাহক সন্তুষ্টই আমাদের সবচেয়ে বড় শক্তি',
        ];

        shuffle($reasons);

        $statistics = [
            ['icon' => 'school.gif', 'target' => 200, 'suffix' => '', 'label' => 'শিক্ষা প্রতিষ্ঠান'],
            ['icon' => 'student.gif', 'target' => 20000, 'suffix' => '+', 'label' => 'শিক্ষার্থী'],
            ['icon' => 'teacher.gif', 'target' => 1500, 'suffix' => '+', 'label' => 'শিক্ষক'],
            ['icon' => 'zila.gif', 'target' => 7, 'suffix' => '', 'label' => 'জেলা'],
            ['icon' => 'upazila.gif', 'target' => 15, 'suffix' => '', 'label' => 'উপজেলা'],
        ];
    ?>
            ...
            <div class="mt-2 grid grid-cols-1 md:grid-cols-2 gap-4 text-slate-600 text-sm">
                <ul class="list-inside">
                    @foreach ($reasons as $index => $reason)
                        @if ($index % 2 == 0)
                            <li class="badge badge-hover p-3 bg-gray-100 rounded-lg shadow-sm hover:bg-gray-200 transition">
                                {{ $reason }}
                            </li>
                        @endif
                    @endforeach
    ...
    <div class="w-5/6 mx-auto flex flex-col lg:flex-row items-center justify-center py-10">
        @foreach ($statistics as $stat)
            <div class="w-1/2 md:w-1/4 p-4 flex flex-col items-center justify-center gap-5 scroll-animate"
                data-animation="fade-in-up" data-delay="{{ $loop->index * 200 }}">
                <img src="{{ asset('icons/' . $stat['icon']) }}" alt="{{ $stat['label'] }} Image"
                    class="w-30 {{ in_array($stat['label'], ['শিক্ষক', 'জেলা', 'উপজেলা']) ? 'scale-150' : '' }}">
                <div class="text-center">
                    <h2 class="text-4xl text-secondary counter" data-target="{{ $stat['target'] }}"
                        data-suffix="{{ $stat['suffix'] }}">0</h2>
                    <p class="text-primary font-bold">{{ $stat['label'] }}</p>
                </div>
            </div>            
        @endforeach
    </div>
    ...
    <div class="w-full max-w-4xl mx-auto mt-20 mb-10 px-4 scroll-animate" data-animation="scale-in">
        <div
            class="relative w-full h-0 pb-[75%] sm:pb-[56.25%] md:pb-[45%] lg:pb-[35%] xl:pb-[30%] min-h-[220px] md:min-h-[280px] lg:min-h-[320px] rounded-lg overflow-hidden shadow-lg">

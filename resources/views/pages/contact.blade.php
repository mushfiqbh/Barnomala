    ...
    <div class="container mx-auto py-12 px-4 mt-10">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <div class="bg-white p-8 rounded-lg shadow-lg">
                ...
                @if (session('success'))
                    <div class="mb-6 rounded-lg border border-green-100 bg-green-50 px-4 py-3 text-green-700">
                        {{ session('success') }}
                    </div>
                @endif

                @if (session('error'))
                    <div class="mb-6 rounded-lg border border-red-100 bg-red-50 px-4 py-3 text-red-600">
                        {{ session('error') }}
                    </div>
                @endif
                ...
                <form action="{{ route('contact.send') }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea id="message" name="message" rows="5" required
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors resize-vertical">{{ old('message') }}</textarea>
                        @error('message')
                            <p class="mt-2 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
